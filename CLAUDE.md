...
**Статус:** MVP готов к публикации в Chrome Web Store — ZIP собран, скриншоты готовы  
**Стек:** Chrome Extension Manifest V3 (ванильный JS, без фреймворков)  
**Последнее обновление:** июнь 2026 (Solana-поддержка, аудит безопасности, CWS-публикация)
...
- ~~Добавить поддержку Solana-адресов в content.js~~ — реализовано: `SOL_RE` base58 {43,44}, `collectMatches()` объединяет EVM+Solana, network пробрасывается в badge и API
- ~~Поддержка брокер-сканирования из popup (ввод домена)~~ — реализовано: `isBrokerDomain()` детектирует домены; `doScan()` роутит на `GET_BROKER_SCORE`; `renderBrokerResult()` показывает score + 3 сигнала (License/Domain/Transparency); `getBrokerScore()` в `background.js` кэширует под `ts_score_broker_*`; placeholder обновлён на "0x… or broker.com"
- ~~Передавать `source: "extension"` в `/analyze`~~ — реализовано: `background.js` уже включает `source: "extension"` в каждый запрос
- ~~Публикация в Chrome Web Store~~ — ZIP готов (`trustscan-extension-v1.0.0.zip`), 4 скриншота в `screenshots/`, store description написан, Privacy Policy на GitHub Pages. Осталось загрузить в Developer Console → ревью 3–7 дней
