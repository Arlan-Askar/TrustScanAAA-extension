...
**Статус:** MVP, работает локально; готов к публикации в Chrome Web Store  
**Стек:** Chrome Extension Manifest V3 (ванильный JS, без фреймворков)  
**Последнее обновление:** июнь 2026 (начальный коммит)
...
- `POST https://trustscan.app/analyze` — основной API-запрос
- Кэш в `chrome.storage.local`: ключ `ts_score_{address}_{network}`, TTL 10 минут
- `API_BASE = "https://trustscan.app"` (в dev: `http://localhost:8080`)
...
- `host_permissions: ["https://trustscan.app/*"]` — доступ к API
...
- Поменять `API_BASE` с `localhost:8080` на `https://trustscan.app` перед публикацией
